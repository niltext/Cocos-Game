module.exports = cc.Class({
    properties: {
        //message RoomInfo
        _eventHandler: null, // 接收自定义event的节点,实为SssView脚本的节点
        roomInfo: null,
    ...
    ctor: function () {
        ...
        net.on(NetId.S2C_SSS_GAME_START, function (event) {
            let msg = event.detail.msg;
            cc.log('199'+msg);
        ...
        net.on(NetId.S2C_SSS_GAME_STATUS, function (event) {
            self.roomInfo.status = event.detail.msg.status;
            cc.log('!++:' + self.roomInfo.status);
        })

        //////////////
        net.on(NetId.S2C_SSS_COOL_DOWN, (event) => {
            if (this.view) {
        ...
        // 玩家下注
        net.on(NetId.S2C_SSS_XIA_ZHU, (event) => {
            this.view.onBet(event.detail.msg);
        });

        // 发真牌
        // net.on(NetId.S2C_SSS_SEND_POKER, function (event) {
        //     if (self.view) {
        ...
        //\\\\\\\\\\\\\
        //小局结算
        net.on(NetId.S2C_SSS_JIESUAN, function (event) {
            if (self.view) {
                self.view.onJiesuan(event.detail.msg);
            }
        })

        net.on(NetId.S2C_SSS_ROOM_MATCH, (event) => {
            let msg = event.detail.msg;
        ...
        net.on(NetId.S2C_SSS_DING_ZHU, (event) => {
            let msg = event.detail.msg;
        ...
        net.on(NetId.S2C_SSS_QIANG_HOST, (event) => {
            let msg = event.detail.msg;
            if (this.view) {
    ...
    onEnterRoom: function (roomInfo, selfInfo, othersInfo, isRejoin) {
    ...
    rejoinRoom: function (msg) {
        ...
        this.onEnterRoom(msg.roomInfo, msg.selfInfo, msg.othersInfo, true);
    },
    ...
    onChat: function (proto) {
        if (this.view) {
            let seat = this.view.getSssSeat(proto.index);
            if (seat) {
    ...
    start: function () {
        cc.mgr.net.send(NetId.C2S_SSS_GAME_START);
    },

    bet: function (_score) {
        cc.log('下注:' + _score);
        cc.mgr.net.send(NetId.C2S_SSS_XIA_ZHU, { score: _score });
    },
    ...
    c2sBanker: function (val) {
        cc.mgr.net.send(NetId.C2S_SSS_QIANG_HOST, { qiang: val });
    },
