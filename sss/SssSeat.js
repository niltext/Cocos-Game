cc.Class({
    ...
    onLoad: function () {
        ...
        avatar_default.on('touchend', (event) => {
            cc.mgr.sss.showPlayerDetail(this._index, this._portrit.spriteFrame);
        });
    },
    ...
    setInfo: function (info) {
        ...
        this.setBanker(info.index == cc.mgr.sss.roomInfo.host_id);
        ...
        this._waiting.active = cc.mgr.sss.isWaiting(this._index);
    ...
    setQiang: function (val, playAnim) {
        ...
        if (playAnim) {
            ...
            let v = '';
            let user = cc.mgr.sss.getUserInfo(this._index);
            if (val) {
    ...
    deal: function (cards) {
        ...
        let myself = (this._index == cc.mgr.sss.myIndex);
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            let moveTo = cc.moveTo(0.2, targetPos.add(cc.v2(i * 10 + 20, 0)));
            ...
            let poker = pokerNode.getComponent('SssPoker');
    ...
    dealImmediately: function (cards) {
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            pokerNode.position = targetPos.add(cc.v2(i * 90 - 180, 0));
            pokerNode.scale = 1;

            let poker = pokerNode.getComponent('SssPoker');
            poker.node.parent = cc.find('Canvas/UI/placePanel/mycard');
            if (cards[i] > 0) {
                poker.turn(cards[i]);
            }
            this._cards.push(poker);
        }
        initCards = this._cards;
        return initCards;
    },
